                ...
                return ListView.builder(
                  ...
                  itemBuilder: (context, index){
                    return ListTile(
                      ...
                      title: Text(dateFormat.parse(bookings[index]["start"]).hour.toString().padLeft(2, '0') + ":" + dateFormat.parse(bookings[index]["start"]).minute.toString().padLeft(2, '0') + " - " + dateFormat.parse(bookings[index]["end"]).hour.toString().padLeft(2, '0') + ":" + dateFormat.parse(bookings[index]["end"]).minute.toString().padLeft(2, '0') + " del " + dateFormat.parse(bookings[index]["end"]).day.toString().padLeft(2, '0') + "/" + dateFormat.parse(bookings[index]["end"]).month.toString().padLeft(2, '0') + "/" + dateFormat.parse(bookings[index]["end"]).year.toString().padLeft(4, '0')),
                      subtitle: Text(parser.emojify(bookings[index]["status"] == "pending" ? ":thumbsup: Confermata" : bookings[index]["status"] == "active" ? ":hourglass_flowing_sand: In corso" : bookings[index]["status"] == "ended" ? ":clock1: Terminata" : ":wastebasket: Cancellata")),
                    );
                  },
                );
              }
